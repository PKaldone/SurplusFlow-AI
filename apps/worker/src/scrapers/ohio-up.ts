...
const SEARCH_URL = 'https://unclaimed.ohio.gov/search';
...
export class OhioUPScraper extends BaseScraper {
  readonly name = 'ohio-up';
  ...
  async scrapeImpl(): Promise<ScrapedOpportunity[]> {
    ...
    try {
      ...
      for (const lastName of namesToSearch) {
        ...
        try {
          await this.throttle();
          this.log(`Searching OH for: ${lastName}`);
          ...
          if (!input) {
            this.errors.push('OH: Could not find search input');
            continue;
          }
          ...
          this.log(`OH: ${results.length} results for "${lastName}"`);
        } catch (err) {
          this.errors.push(`OH "${lastName}": ${err instanceof Error ? err.message : String(err)}`);
        }
      }
    } finally {
      await browser.close();
      ...
      if (cells.length >= 2) {
        ...
        if (name && !isNaN(amount) && amount > 0) {
          results.push({
            ...
            source_id: `OH-UP-${name.replace(/\s+/g, '-')}-${amount}`,
