...
const SEARCH_URL = 'https://ouf.osc.ny.gov/ouf/';

const SEARCH_NAMES = [
  'Smith', 'Johnson', 'Williams', 'Brown', 'Jones', 'Garcia', 'Miller',
  'Davis', 'Rodriguez', 'Martinez', 'Lopez', 'Wilson', 'Anderson',
  'Lee', 'Chen', 'Wang', 'Kim', 'Patel', 'Singh', 'Cohen',
];

export class NewYorkUPScraper extends BaseScraper {
  readonly name = 'newyork-up';
  ...
  async scrapeImpl(): Promise<ScrapedOpportunity[]> {
    ...
    try {
      ...
      for (const lastName of namesToSearch) {
        ...
        try {
          await this.throttle();
          this.log(`Searching NY for: ${lastName}`);
          ...
          if (!input) {
            this.errors.push('NY: Could not find search input');
            continue;
          }
          ...
          this.log(`NY: ${results.length} results for "${lastName}"`);
        } catch (err) {
          this.errors.push(`NY "${lastName}": ${err instanceof Error ? err.message : String(err)}`);
        }
      }
    } finally {
      await browser.close();
      ...
      if (cells.length >= 2) {
        ...
        if (name && !isNaN(amount) && amount > 0) {
          results.push({
            ...
            source_id: `NY-UP-${name.replace(/\s+/g, '-')}-${amount}`,
